...
//* 対戦管理プログラム．
//* 実行方法は，
// ./match プレーヤーAプログラム プレーヤーBプログラム ゲーム数
//* 各プレーヤープログラムを子プロセスとして実行．この時，ゲーム数を引数として子プロセスに渡す．
//* 対戦管理プログラムは，各プレーヤープログラムと賭金を双方向パイプで通信．
//* 異常終了などしたときに，子プロセスがゾンビ化することを避けるため，若干複雑な処理をしている．
 ...
 int main(int argc,char **argv){
   ...
   exit(0);  // to avoid Warning message.
 }
